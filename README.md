...
- 可从设置或系统入口通过 FlatStore 公开版本接口手动检查更新；确认后下载对应平台安装包，校验通过再调用系统打开。
...
XDecode 以菜单栏 App 运行，不在程序坞或 `Command-Tab` 中显示。菜单栏小图标始终保留，可用于打开主窗口、选择日志、检查更新、进入设置和退出 App。
...
`~/Downloads` 使用 macOS 下载目录专用沙盒权限。第一次处理其他目录中的文件时，App Sandbox 会要求授权日志所在文件夹。“开机自启动”默认开启，可在设置中独立关闭；它与“自动解密”互不影响。“常规”设置会显示当前版本并提供“检查更新”，该操作只请求 XDecode 的 FlatStore 公开版本元数据；仅在用户确认后下载对应平台的安装包，校验通过再调用系统打开。
...
- `XDecodeApp/UpdateChecker.swift`：FlatStore 公开版本查询、语义版本比较和安装包下载校验。
